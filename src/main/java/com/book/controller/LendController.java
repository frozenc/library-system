...
public class LendController {
    ...
    public String deletelendlist(HttpServletRequest request,RedirectAttributes redirectAttributes){
        long lendlistId=Integer.parseInt(request.getParameter("lendlistId"));
        boolean retSucc=lendService.deletelendlist(lendlistId);
        if (retSucc){
            redirectAttributes.addFlashAttribute("succ", "删除日志成功！");
            return "redirect:/lendlist.html";
        }
        else {
            redirectAttributes.addFlashAttribute("error", "删除日志失败！");
            return "redirect:/lendlist.html";
        }
    }
